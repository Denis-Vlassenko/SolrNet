...
namespace SolrNet.Cloud
{
    ...
    public class SolrCloudServer2<T>
        : ISolrCloudOperations<T> {

        public SolrCloudServer2(string zkHost, string collectionName) {
            reader = new ZkStateReader();
            state = reader.GetClusterState(zkHost, collectionName);
            this.zkHost = zkHost;
        ...
        private readonly CollectionClusterState state;
        private string zkHost;

        private TResult Perform<TResult>(Func<ISolrOperations<T>,  TResult> operation, bool runOnLeader = false) {

            var urls = runOnLeader ? state.LeadersUrls : state.ActiveUrls;

            // decide which url to take randomly
            int rnd = new Random().Next(urls.Count);
            var url = urls[rnd];

            try {
                return operation(GetSolrOperations(url));
        ...
        private ISolrOperations<T> GetSolrOperations(string url) {
            // cache?
